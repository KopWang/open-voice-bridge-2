...
“组合按键”区域可以选择两个遥控器按键，并把它们同时按下映射成另一个快捷键。配置过显式组合的成员键会等待 140ms 判断组合；没有形成组合时仍执行各自原映射。遥控器必须在同一份 HID 报告中同时上报两个按键，应用不会把先后两次点击伪装成同时按下。

RC003 实测会把修饰键和随后按下的方向键串行上报。为保持真实键盘输出语义，单个修饰键松开后有 500ms 跟随窗口；窗口内出现下一个遥控器按键时，修饰键输出保持到该按键释放，macOS 因而收到重叠的按下区间，而不是两个独立点击。
